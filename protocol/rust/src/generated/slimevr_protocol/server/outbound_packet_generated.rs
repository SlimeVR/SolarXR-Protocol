...
impl<'a> OutboundPacket<'a> {
  ...
  pub fn packet_as_trackers_list(&self) -> Option<TrackersList<'a>> {
    if self.packet_type() == OutboundUnion::TrackersList {
      self.packet().map(TrackersList::init_from_table)
    } else {
      None
    }
  }
  ...
  #[inline]
  #[allow(non_snake_case)]
  pub fn packet_as_settings_response(&self) -> Option<SettingsResponse<'a>> {
    if self.packet_type() == OutboundUnion::SettingsResponse {
      self.packet().map(SettingsResponse::init_from_table)
    } else {
      None
    }
  }
  ...
  fn run_verifier(
    ...
    v.visit_table(pos)?
     ...
     .visit_union::<OutboundUnion, _>("packet_type", Self::VT_PACKET_TYPE, "packet", Self::VT_PACKET, false, |key, v, pos| {
        match key {
          OutboundUnion::TrackersList => v.verify_union_variant::<flatbuffers::ForwardsUOffset<TrackersList>>("OutboundUnion::TrackersList", pos),
          OutboundUnion::SettingsResponse => v.verify_union_variant::<flatbuffers::ForwardsUOffset<SettingsResponse>>("OutboundUnion::SettingsResponse", pos),
    ...
    let mut ds = f.debug_struct("OutboundPacket");
      ...
      match self.packet_type() {
        OutboundUnion::TrackersList => {
          if let Some(x) = self.packet_as_trackers_list() {
            ds.field("packet", &x)
          } else {
        ...
        OutboundUnion::SettingsResponse => {
          if let Some(x) = self.packet_as_settings_response() {
            ds.field("packet", &x)
          } else {
            ds.field("packet", &"InvalidFlatbuffer: Union discriminant does not match value.")
          }
