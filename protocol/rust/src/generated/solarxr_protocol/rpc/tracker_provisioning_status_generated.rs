// automatically generated by the FlatBuffers compiler, do not modify
// @generated
extern crate alloc;
extern crate flatbuffers;
use alloc::boxed::Box;
use alloc::string::{String, ToString};
use alloc::vec::Vec;
use core::mem;
use core::cmp::Ordering;
use self::flatbuffers::{EndianScalar, Follow};
use super::*;
#[deprecated(since = "2.0.0", note = "Use associated constants instead. This will no longer be generated in 2021.")]
pub const ENUM_MIN_TRACKER_PROVISIONING_STATUS: u8 = 0;
#[deprecated(since = "2.0.0", note = "Use associated constants instead. This will no longer be generated in 2021.")]
pub const ENUM_MAX_TRACKER_PROVISIONING_STATUS: u8 = 8;
#[deprecated(since = "2.0.0", note = "Use associated constants instead. This will no longer be generated in 2021.")]
#[allow(non_camel_case_types)]
pub const ENUM_VALUES_TRACKER_PROVISIONING_STATUS: [TrackerProvisioningStatus; 9] = [
  TrackerProvisioningStatus::SERIAL_INIT,
  TrackerProvisioningStatus::OBTAINING_MAC_ADDRESS,
  TrackerProvisioningStatus::PROVISIONING,
  TrackerProvisioningStatus::CONNECTING,
  TrackerProvisioningStatus::CONNECTION_ERROR,
  TrackerProvisioningStatus::LOOKING_FOR_SERVER,
  TrackerProvisioningStatus::COULD_NOT_FIND_SERVER,
  TrackerProvisioningStatus::DONE,
  TrackerProvisioningStatus::NO_SERIAL_LOGS_ERROR,
];

#[derive(Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Default)]
#[repr(transparent)]
pub struct TrackerProvisioningStatus(pub u8);
#[allow(non_upper_case_globals)]
impl TrackerProvisioningStatus {
  pub const SERIAL_INIT: Self = Self(0);
  pub const OBTAINING_MAC_ADDRESS: Self = Self(1);
  pub const PROVISIONING: Self = Self(2);
  pub const CONNECTING: Self = Self(3);
  pub const CONNECTION_ERROR: Self = Self(4);
  pub const LOOKING_FOR_SERVER: Self = Self(5);
  pub const COULD_NOT_FIND_SERVER: Self = Self(6);
  pub const DONE: Self = Self(7);
  pub const NO_SERIAL_LOGS_ERROR: Self = Self(8);

  pub const ENUM_MIN: u8 = 0;
  pub const ENUM_MAX: u8 = 8;
  pub const ENUM_VALUES: &'static [Self] = &[
    Self::SERIAL_INIT,
    Self::OBTAINING_MAC_ADDRESS,
    Self::PROVISIONING,
    Self::CONNECTING,
    Self::CONNECTION_ERROR,
    Self::LOOKING_FOR_SERVER,
    Self::COULD_NOT_FIND_SERVER,
    Self::DONE,
    Self::NO_SERIAL_LOGS_ERROR,
  ];
  /// Returns the variant's name or "" if unknown.
  pub fn variant_name(self) -> Option<&'static str> {
    match self {
      Self::SERIAL_INIT => Some("SERIAL_INIT"),
      Self::OBTAINING_MAC_ADDRESS => Some("OBTAINING_MAC_ADDRESS"),
      Self::PROVISIONING => Some("PROVISIONING"),
      Self::CONNECTING => Some("CONNECTING"),
      Self::CONNECTION_ERROR => Some("CONNECTION_ERROR"),
      Self::LOOKING_FOR_SERVER => Some("LOOKING_FOR_SERVER"),
      Self::COULD_NOT_FIND_SERVER => Some("COULD_NOT_FIND_SERVER"),
      Self::DONE => Some("DONE"),
      Self::NO_SERIAL_LOGS_ERROR => Some("NO_SERIAL_LOGS_ERROR"),
      _ => None,
    }
  }
}
impl core::fmt::Debug for TrackerProvisioningStatus {
  fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
    if let Some(name) = self.variant_name() {
      f.write_str(name)
    } else {
      f.write_fmt(format_args!("<UNKNOWN {:?}>", self.0))
    }
  }
}
impl<'a> flatbuffers::Follow<'a> for TrackerProvisioningStatus {
  type Inner = Self;
  #[inline]
  unsafe fn follow(buf: &'a [u8], loc: usize) -> Self::Inner {
    let b = flatbuffers::read_scalar_at::<u8>(buf, loc);
    Self(b)
  }
}

impl flatbuffers::Push for TrackerProvisioningStatus {
    type Output = TrackerProvisioningStatus;
    #[inline]
    unsafe fn push(&self, dst: &mut [u8], _written_len: usize) {
        flatbuffers::emplace_scalar::<u8>(dst, self.0);
    }
}

impl flatbuffers::EndianScalar for TrackerProvisioningStatus {
  type Scalar = u8;
  #[inline]
  fn to_little_endian(self) -> u8 {
    self.0.to_le()
  }
  #[inline]
  #[allow(clippy::wrong_self_convention)]
  fn from_little_endian(v: u8) -> Self {
    let b = u8::from_le(v);
    Self(b)
  }
}

impl<'a> flatbuffers::Verifiable for TrackerProvisioningStatus {
  #[inline]
  fn run_verifier(
    v: &mut flatbuffers::Verifier, pos: usize
  ) -> Result<(), flatbuffers::InvalidFlatbuffer> {
    use self::flatbuffers::Verifiable;
    u8::run_verifier(v, pos)
  }
}

impl flatbuffers::SimpleToVerifyInSlice for TrackerProvisioningStatus {}
