...
public data class MessageBundle(
  ...
  public fun encode(builder: FlatBufferWriter): Int {
    ...
    __off_connectionMsgs?.let { builder.addOffset(0, it, 0) }
    __off_dataFeedMsgs?.let { builder.addOffset(1, it, 0) }
    __off_rpcMsgs?.let { builder.addOffset(2, it, 0) }
    __off_driverMsgs?.let { builder.addOffset(3, it, 0) }
  ...
  public companion object {
    public fun decode(bb: FlatBufferReader, tableOffset: Int): MessageBundle {
      ...
      val __offset_connectionMsgs = if (vtableSize > 4) bb.getShort(vtableOffset + 4).toInt() else 0
      val __offset_dataFeedMsgs = if (vtableSize > 6) bb.getShort(vtableOffset + 6).toInt() else 0
      val __offset_rpcMsgs = if (vtableSize > 8) bb.getShort(vtableOffset + 8).toInt() else 0
      val __offset_driverMsgs = if (vtableSize > 10) bb.getShort(vtableOffset + 10).toInt() else 0
