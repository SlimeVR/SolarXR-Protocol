...
public sealed interface PubSubUnion {
  public companion object {
    ...
    public fun typeIndex(`value`: PubSubUnion): Byte = when (value) {
      is Message -> 1
    ...
    public fun encode(`value`: PubSubUnion, builder: FlatBufferWriter): Int = when (value) {
...
public data class PubSubHeader(
  public val u: PubSubUnion? = null,
) {
  ...
  public companion object {
    public fun decode(bb: FlatBufferReader, tableOffset: Int): PubSubHeader {
      ...
      return PubSubHeader(
              u = if (__offset_u != 0) PubSubUnion.decode(__type_u, bb, tableOffset + __offset_u + bb.getInt(tableOffset + __offset_u)) else null
          )
    }
  }
}
...
public interface Payload {
  public companion object {
    ...
    public fun typeIndex(`value`: Payload): Byte = when (value) {
      is StringTable -> 1
      is Bytes -> 2
      is KeyValues -> 3
      else -> 0
    ...
    public fun encode(`value`: Payload, builder: FlatBufferWriter): Int = when (value) {
...
public data class Message(
  ...
  public companion object {
    public fun decode(bb: FlatBufferReader, tableOffset: Int): Message {
      val vtableOffset = tableOffset - bb.getInt(tableOffset)
      val vtableSize = bb.getShort(vtableOffset).toInt()
      ...
      return Message(
              ...
              payload = if (__offset_payload != 0) Payload.decode(__type_payload, bb, tableOffset + __offset_payload + bb.getInt(tableOffset + __offset_payload)) else null
          )
    }
  }
}

public data class KeyValues(
  ...
  public companion object {
    public fun decode(bb: FlatBufferReader, tableOffset: Int): KeyValues {
      val vtableOffset = tableOffset - bb.getInt(tableOffset)
      val vtableSize = bb.getShort(vtableOffset).toInt()
      ...
      return KeyValues(
              keys = if (__offset_keys != 0) { val vecOff = tableOffset + __offset_keys + bb.getInt(tableOffset + __offset_keys); val len = bb.getInt(vecOff); (0 until len).mapNotNull { i -> readFlatBufferString(bb, vecOff + 4 + i * 4) } } else null,
              values = if (__offset_values != 0) { val vecOff = tableOffset + __offset_values + bb.getInt(tableOffset + __offset_values); val len = bb.getInt(vecOff); (0 until len).mapNotNull { i -> readFlatBufferString(bb, vecOff + 4 + i * 4) } } else null
          )
    }
  }
}
